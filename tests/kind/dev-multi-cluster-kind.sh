...
EOF
      ...
      # Each cluster needs its own network for multi-cluster setup
      cluster_network="${network_name}-${i}"
...
- |-
  [plugins."io.containerd.grpc.v1.cri".registry.mirrors."localhost:${REGISTRY}"]
    endpoint = ["http://${cluster_network}-registry:5000"]
EOF
...
EOF
      ...
      # Connect cluster to custom network for multi-cluster communication
      if [[ "$clusters" -gt 1 ]]; then
        # Create network for multi-cluster setup if it doesn't exist
        if ! docker network inspect "${cluster_network}" >/dev/null 2>&1; then
          echo "Creating network: ${cluster_network}"
          docker network create "${cluster_network}"
        fi
        
        # Connect all nodes to the custom network
