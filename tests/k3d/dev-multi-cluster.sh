...
# Function to install Cilium binary on K3D clusters.
install_cilium() {
  # Path to the Cilium binary.
  CILIUM_PATH="${CILIUM_PATH:-cilium}" # Use "cilium" if CILIUM_PATH is not set

  for ((i=0; i<clusters; i++)); do
    cluster_name="${name}-cluster-${i}"
    cluster_id="$i" # Cluster ID is just the loop counter for this example.  Adjust as needed.

    # Determine the context name
    context_name="k3d-${cluster_name}"

    # Install Cilium using the binary
    #TODO make kubeProxyReplacement as true or false
    "$CILIUM_PATH" install \
    --set ipam.operator.clusterPoolCIDR=192.168.0.0/16 \
    --set ipam.operator.podCIDR=192.168.0.0/16 \
    --set ipam.mode=cluster-pool  --set cluster-pool-ipv4-cidr=192.168.0.0/24  --set operator.serviceLBIPRange=192.168.0.0/24 \
    ...
    if [[ $? -eq 0 ]]; then
      echo "Cilium installed on ${cluster_name} (context: ${context_name})"
    else
      echo "Error installing Cilium on ${cluster_name} (context: ${context_name})"
      return 1 # Return an error code if installation fails
    fi
