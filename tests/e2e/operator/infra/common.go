...
// UpdateCoreDNSForNamespaces updates CoreDNS configuration with new namespaces when reusing infrastructure.
func UpdateCoreDNSForNamespaces(t *testing.T, r *operator.Region, getClusterStaticIP func(i int) string) {
	kubeConfigPath, err := r.EnsureKubeConfigPath()
	require.NoError(t, err)

	// Ensure CorednsClusterOptions is initialized.
	if r.CorednsClusterOptions == nil {
		r.CorednsClusterOptions = make(map[string]coredns.CoreDNSClusterOption)
	}

	// Update CoreDNS cluster options with new namespaces
	for i, clusterName := range r.Clusters {
		// Get the IP for this cluster (maybe static or placeholder for dynamic)
		clusterIP := getClusterStaticIP(i)
		if clusterIP == "" {
			// Use placeholder IP for dynamic allocation - will be updated with actual IP later.
			clusterIP = "127.0.0.1"
		}
		// Update the cluster options with current namespaces.
		r.CorednsClusterOptions[operator.CustomDomains[i]] = coredns.CoreDNSClusterOption{
			IPs:       []string{clusterIP},
