...
// ─── ENCRYPTION PROVIDER INTERFACE IMPLEMENTATION ───────────────────────────
...
// SetupEncryptionInfrastructure is a no-op for AWS (using file-based encryption for now)
// TODO: Implement full AWS KMS support with key creation, IAM roles, and policies
// Returns a no-op cleanup function
func (r *AwsRegion) SetupEncryptionInfrastructure(t *testing.T) (func(), error) {
	t.Log("AWS provider: No KMS infrastructure needed for file-based encryption (UNKNOWN_KEY_TYPE)")
	t.Log("TODO: Implement AWS KMS key creation, IAM role setup, and policies for production-grade encryption")
	// Return no-op cleanup function
	return func() {
		t.Log("AWS provider: No KMS infrastructure to clean up")
	}, nil
}

// GetEncryptionPlatformConfig returns file-based encryption platform configuration for AWS
// TODO: Change to AWS_KMS once KMS infrastructure is implemented
func (r *AwsRegion) GetEncryptionPlatformConfig() *encryption.PlatformConfig {
	return &encryption.PlatformConfig{
		Platform:                     "UNKNOWN_KEY_TYPE", // TODO: Change to "AWS_KMS" when KMS is implemented
		RequiresCredentialsSecret:    false,              // TODO: Change to true for AWS_KMS
		DefaultCredentialsSecretName: "",                 // TODO: Set to "aws-kms-credentials" for AWS_KMS
	}
}

// GetCMEKSecretSetup returns nil for AWS since it currently uses file-based encryption (UNKNOWN_KEY_TYPE)
// TODO: Implement full AWS Cloud KMS support and return CMEKSecretSetup with AWS KMS callbacks
func (r *AwsRegion) GetCMEKSecretSetup() *encryption.CMEKSecretSetup {
	// Return nil for file-based encryption (UNKNOWN_KEY_TYPE)
	// The generic SetupEncryptionSecrets function will detect nil and use SetupFileBasedEncryptionSecrets
	return nil
}

// SetupEncryptionSecrets creates Kubernetes secrets for file-based encryption (UNKNOWN_KEY_TYPE)
// TODO: Implement AWS KMS-based encryption with envelope encryption
func (r *AwsRegion) SetupEncryptionSecrets(t *testing.T, kubectlOptions *k8s.KubectlOptions, clusterRegion string) error {
	// Use shared utility for file-based encryption (UNKNOWN_KEY_TYPE)
	// TODO: Implement AWS KMS envelope encryption:
	//  1. Generate data encryption key (DEK)
	//  2. Encrypt DEK with AWS KMS key
	//  3. Create secret with encrypted DEK and KMS key ARN
	//  4. Create IAM credentials secret if needed
	return encryption.SetupFileBasedEncryptionSecrets(t, kubectlOptions, ProviderAWS)
}
...
func (r *AwsRegion) createEKSClusters(
	...
	for i, clusterName := range r.Clusters {
		go func(idx int, name string) {
			...
			if err := operator.EnsureKubeconfigTLSVerificationDisabled(t, []string{name}); err != nil {
...
func (r *AwsRegion) createEKSCluster(
	...
	if err := cmd.Run(); err != nil {
		...
		if describeErr == nil && describeResp.Cluster != nil {
			...
			if err := operator.EnsureKubeconfigTLSVerificationDisabled(t, []string{setup.ClusterName}); err != nil {
				t.Logf("[%s] Warning: Failed to apply TLS settings: %v", ProviderAWS, err)
			}
