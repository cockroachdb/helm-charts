...
func (r *GcpRegion) SetUpInfra(t *testing.T) {
	...
	// 3) Create subnets & discover zones
	...
	for i, _ := range r.Clusters {
		...
		// We store the allocation ID in the StaticIPAddress field for later reference
	...
	for i, clusterName := range r.Clusters {
		...
		// 6d) Prepare controller-runtime client for this context
...
func getServiceAccountKeyPath() string {
	// First check if explicitly set in code
	...
	// Finally check if running in GitHub Actions
...
func ensureStaticIPAddress(ctx context.Context, client *compute.Service, projectID, region, addressName, addressIP, subnetSelfLink string) (string, error) {
	...
	// 2) Create new reserved internal IP
...
func createGKERegionalCluster(ctx context.Context, client *container.Service, setup ClusterSetupConfig,
	...
	// Defensive Check: See if the cluster already exists.
	...
	args := []string{
		...
		"--region", setup.Region,
		"--project", projectID, // Explicitly pass project to avoid ambiguity
		"--network", vpcSelfLink,
		"--subnetwork", subnetSelfLink,
	...
	// Stream gcloud's stdout/stderr directly to your process's stdout/stderr
	// for real-time visibility into the long-running creation process.
