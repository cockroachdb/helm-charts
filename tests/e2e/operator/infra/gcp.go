...
func (r *GcpRegion) SetUpInfra(t *testing.T) {
	if r.ReusingInfra {
		...
		// Update CoreDNS configuration with new namespaces even when reusing infrastructure.
		UpdateCoreDNSForNamespaces(t, r.Region, func(i int) string {
			if i < len(clusterConfigurations) {
				return clusterConfigurations[i].StaticIPAddress
			}
			return ""
		})
		return
