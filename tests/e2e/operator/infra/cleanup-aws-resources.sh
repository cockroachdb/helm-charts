...
#   - KMS keys and aliases (scheduled for deletion with 7-day waiting period)
#   - IAM roles for KMS encryption (global)
#   - IAM users for KMS encryption (global)
#   - IAM roles for EBS CSI driver and attached policies
...
#
# Usage:
#   ./cleanup-aws-resources.sh [--dry-run] --cluster <CLUSTER_NAME> [--noconsent]  # Safe - deletes specific cluster
...
#
# Examples:
#   ./cleanup-aws-resources.sh --cluster aws-chart-testing-cluster-0-abc123        # Delete specific cluster
#   ./cleanup-aws-resources.sh --dry-run --cluster aws-chart-testing-cluster-0-abc123  # Show what would be deleted
#   AWS_REGION=us-east-1 ./cleanup-aws-resources.sh --cluster aws-chart-testing-cluster-0-abc123  # Delete only in us-east-1
...
if [ $# -eq 0 ]; then
    ...
    echo ""
    echo "Usage:"
    echo "  $0 [--dry-run] --cluster <CLUSTER_NAME> [--noconsent]           # Delete specific cluster (SAFE)"
    echo "  $0 [--dry-run] --all [--noconsent]                              # Delete ALL e2e resources (DANGEROUS)"
    echo "  $0 [--dry-run] --before-date <MM/DD/YYYY> --all [--noconsent]   # Delete old resources"
    echo ""
    echo "Examples:"
...
# Validate arguments
if [ "$DELETE_ALL" = false ] && [ -z "$CLUSTER_NAME" ]; then
    echo "Error: Must specify either --all or --cluster <name>"
    exit 1
fi
...
for provider_arn in $all_oidc_providers; do
    ...
    if [[ $provider_arn == *"oidc.eks."*".amazonaws.com"* ]]; then
        ...
        if [[ $provider_arn == *"oidc.eks.$REGION.amazonaws.com"* ]]; then
            ...
            if [ -n "$CLUSTER_NAME" ]; then
                # In targeted mode, check if OIDC provider matches our cluster
...
# 1b. Delete KMS keys for encryption at rest
# KMS keys cannot be deleted immediately - AWS enforces a 7-30 day waiting period
# This step schedules them for deletion and removes their aliases
if [ "$DRY_RUN" = false ]; then
    echo "Step 1b: Scheduling KMS keys for deletion in region $REGION..."
else
    echo "Step 1b: Finding KMS keys in region $REGION..."
fi

# List all KMS keys in the region
kms_keys=$(aws kms list-keys --region "$REGION" --query 'Keys[].KeyId' --output text 2>/dev/null || true)

for key_id in $kms_keys; do
    # Get key metadata including tags
    key_metadata=$(aws kms describe-key --region "$REGION" --key-id "$key_id" --query 'KeyMetadata' --output json 2>/dev/null || echo "{}")
    key_state=$(echo "$key_metadata" | jq -r '.KeyState // ""')
    key_arn=$(echo "$key_metadata" | jq -r '.Arn // ""')

    # Skip if key is already scheduled for deletion or disabled
    if [ "$key_state" = "PendingDeletion" ] || [ "$key_state" = "Disabled" ]; then
        continue
    fi

    # Get tags for this key
    tags=$(aws kms list-resource-tags --region "$REGION" --key-id "$key_id" --query 'Tags' --output json 2>/dev/null || echo "[]")
    managed_by=$(echo "$tags" | jq -r '.[] | select(.TagKey=="ManagedBy") | .TagValue' 2>/dev/null || echo "")

    # Only process keys managed by helm-charts-e2e
    if [ "$managed_by" != "helm-charts-e2e" ]; then
        continue
    fi

    # Check if key is old enough to delete (if --before-date specified)
    if [ -n "$BEFORE_DATE" ]; then
        # Extract creation date from key metadata
        key_creation=$(echo "$key_metadata" | jq -r '.CreationDate // ""')
        key_timestamp=$(iso8601_to_timestamp "$key_creation")
        if ! is_resource_old_enough "$key_timestamp"; then
            if [ "$DRY_RUN" = false ]; then
                echo "Skipping KMS key (too new): $key_id (created: $key_creation)"
            else
                echo "[DRY RUN] Would skip KMS key (too new): $key_id (created: $key_creation)"
            fi
            continue
        fi
    fi
    ...
    if [ "$DRY_RUN" = false ]; then
        echo "Scheduling KMS key for deletion: $key_id (7-day waiting period)"

        # Delete all aliases for this key first
        aliases=$(aws kms list-aliases --region "$REGION" --key-id "$key_id" --query 'Aliases[].AliasName' --output text 2>/dev/null || true)
        for alias in $aliases; do
            if [[ $alias == alias/cockroachdb-e2e-* ]]; then
        ...
        aws kms schedule-key-deletion \
            --region "$REGION" \
            --key-id "$key_id" \
            --pending-window-in-days 7 2>/dev/null || echo "  Failed to schedule deletion (may already be scheduled)"
    else
        echo "[DRY RUN] Would schedule KMS key for deletion: $key_id"

        # Show aliases that would be deleted
        aliases=$(aws kms list-aliases --region "$REGION" --key-id "$key_id" --query 'Aliases[].AliasName' --output text 2>/dev/null || true)
        for alias in $aliases; do
            if [[ $alias == alias/cockroachdb-e2e-* ]]; then
                echo "  Would delete KMS alias: $alias"
            fi
        done
    fi
done

# 1c. Delete IAM roles for KMS encryption (global resource, but only process once per script run)
# Use a flag to ensure we only process global IAM resources once across all regions
if [ "$REGION" = "${REGIONS[0]}" ]; then
    if [ "$DRY_RUN" = false ]; then
        echo "Step 1c: Deleting global IAM roles for KMS encryption..."
    else
        echo "Step 1c: Finding global IAM roles for KMS encryption..."
    fi

    # Find IAM roles matching pattern cockroachdb-kms-e2e-*
    all_roles=$(aws iam list-roles --query 'Roles[?starts_with(RoleName, `cockroachdb-kms-e2e-`)].RoleName' --output text 2>/dev/null || true)
    ...
    # 1d. Delete IAM users for KMS encryption (global resource)
    if [ "$DRY_RUN" = false ]; then
        echo "Step 1d: Deleting global IAM users for KMS encryption..."
    else
        echo "Step 1d: Finding global IAM users for KMS encryption..."
    fi

    # Find IAM users matching pattern cockroachdb-kms-user-*
    all_users=$(aws iam list-users --query 'Users[?starts_with(UserName, `cockroachdb-kms-user-`)].UserName' --output text 2>/dev/null || true)

    for user_name in $all_users; do
        # Get user tags
        tags=$(aws iam list-user-tags --user-name "$user_name" --query 'Tags' --output json 2>/dev/null || echo "[]")
        managed_by=$(echo "$tags" | jq -r '.[] | select(.Key=="ManagedBy") | .Value' 2>/dev/null || echo "")
        ...
        if [ "$managed_by" = "helm-charts-e2e" ]; then
            if [ "$DRY_RUN" = false ]; then
                ...
                for access_key_id in $access_keys; do
                    [ -n "$access_key_id" ] && aws iam delete-access-key \
                        --user-name "$user_name" \
                        --access-key-id "$access_key_id" 2>/dev/null || true
                done

                # Delete all inline policies
                inline_policies=$(aws iam list-user-policies --user-name "$user_name" --query 'PolicyNames' --output text 2>/dev/null || true)
                for policy_name in $inline_policies; do
                    [ -n "$policy_name" ] && aws iam delete-user-policy \
                        --user-name "$user_name" \
                        --policy-name "$policy_name" 2>/dev/null || true
                done
                ...
                for policy_arn in $attached_policies; do
                    [ -n "$policy_arn" ] && aws iam detach-user-policy \
                        --user-name "$user_name" \
                        --policy-arn "$policy_arn" 2>/dev/null || true
                done
                ...
                echo "[DRY RUN] Would delete IAM user: $user_name"
            fi
        fi
    done
fi

# 1e. Delete IAM roles for EBS CSI driver
if [ "$DRY_RUN" = false ]; then
    echo "Step 1e: Deleting IAM roles for EBS CSI driver in region $REGION..."
else
    echo "Step 1e: Finding IAM roles for EBS CSI driver in region $REGION..."
fi

if [ -n "$CLUSTER_NAME" ]; then
    # Find IAM roles matching pattern ebs-csi-{region}-* that match the cluster name pattern
    # Note: Without TestRunID, we can only match by role naming pattern
    all_roles=$(aws iam list-roles --query 'Roles[?starts_with(RoleName, `ebs-csi-'$REGION'-`)].RoleName' --output text 2>/dev/null || true)

    for role_name in $all_roles; do
        # Get role tags to verify it's managed by helm-charts-e2e
        tags=$(aws iam list-role-tags --role-name "$role_name" --query 'Tags' --output json 2>/dev/null || echo "[]")
        managed_by=$(echo "$tags" | jq -r '.[] | select(.Key=="ManagedBy") | .Value' 2>/dev/null || echo "")

        # Only process if it has our ManagedBy tag
        if [ "$managed_by" = "helm-charts-e2e" ]; then
            if [ "$DRY_RUN" = false ]; then
                echo "Deleting IAM role: $role_name"
...
for stack in $stacks; do
    ...
    # If --all mode and no cluster name, check if it's a helm-charts-e2e stack
...
# ONLY delete EIPs explicitly tagged as managed by helm-charts-e2e
