...
func (r *LocalRegion) SetUpInfra(t *testing.T) {
	...
	for i, cluster := range r.Clusters {
		...
		t.Logf("[%s] CoreDNS service assigned LoadBalancer IP(s): %v", r.ProviderType, ips)
	...
	for i, cluster := range r.Clusters {
		...
		// Scale down existing kube-dns to prevent conflicts
		// Note: We don't scale down "coredns" deployment because that's our custom CoreDNS deployment
		t.Logf("[%s] Scaling down default kube-dns in cluster %s", r.ProviderType, cluster)
		_ = k8s.RunKubectlE(t, kubectlOptions, "scale", "deployment", "kube-dns-autoscaler", "--replicas=0")
		_ = k8s.RunKubectlE(t, kubectlOptions, "scale", "deployment", "kube-dns", "--replicas=0")

		// Restart our custom coredns deployment to pick up the updated ConfigMap
	...
	if r.IsMultiRegion && (r.ProviderType == ProviderK3D || r.ProviderType == ProviderKind) {
		...
		t.Logf("[%s] Skipping BGP peering setup (either single region or provider not using Calico)", r.ProviderType)
	}
}
