...
func mergeOpenShiftKubeconfig(t *testing.T, generatedKubeconfig, clusterAlias string) error {
	...
	// Put generatedKubeconfig first so its credentials (user "admin") take
	// precedence over any existing "admin" user entry in destKubeconfig when
	// kubectl flattens conflicting keys.
	merged := fmt.Sprintf("%s:%s", generatedKubeconfig, destKubeconfig)
