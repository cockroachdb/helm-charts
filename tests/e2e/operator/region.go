...
const (
	CloudProvider = "k3d"
	CoreDNSImage  = "coredns/coredns:1.9.2"
	TestDBName    = "testdb"
...
var setupNetworking func(ctx context.Context, region string, netConfig calico.K3dCalicoBGPPeeringOptions, options *k8s.KubectlOptions, clusterId int) error
...
func (r *Region) SetUpInfra(t *testing.T, corednsClusterOptions map[string]coredns.CoreDNSClusterOption) {
	...
	for i, cluster := range Clusters {
		...
		// Apply deployment.
		deploymentYaml := coredns.ToYamlDeployment(t, deployment)
		...
		serviceYaml := coredns.ToYamlService(t, service)
		// Apply service.
	...
	for _, cluster := range Clusters {
		...
		cmYaml := coredns.ToYamlConfigmap(t, cm)
	...
	// service discovery.
	setupNetworking = func(ctx context.Context, region string, netConfig calico.K3dCalicoBGPPeeringOptions, options *k8s.KubectlOptions, clusterId int) error {
		...
		ctl := clients[options.ContextName]
	...
	for i, region := range RegionCodes {
		...
		err := setupNetworking(context.TODO(), region, netConfig, kubectlOptions, i)
		if err != nil {
			t.Error(err)
		}
	}
...
// InstallCharts Installs both Operator and CRDB charts by providing custom CA secret
...
func (r *Region) InstallCharts(t *testing.T, cluster string, index int) {
	...
	crdbOptions := &helm.Options{
		...
		SetValues: PatchHelmValues(map[string]string{
			...
			"operator.certificates.externalCertificates.clientCaConfigMapName":       customCASecret + "-crt",
			"operator.certificates.externalCertificates.nodeCaConfigMapName":         customCASecret + "-crt",
	...
	serviceName := fmt.Sprintf("cockroachdb-public")
...
func (r *Region) ValidateCRDB(t *testing.T, cluster string) {
	...
	testutil.ValidateCerts(t, crdbCluster, cluster)

	pods := k8s.ListPods(t, kubectlOptions, metav1.ListOptions{
		LabelSelector: LabelSelector,
	})
	require.True(t, len(pods) > 0)
	//podName := fmt.Sprintf("%s.cockroachdb", pods[0].Name)
	//Todo(nishanth): not working for multi-region, have to debug.
...
func (r *Region) VerifyHelmUpgrade(t *testing.T, initialTimestamp time.Time, kubectlOptions *k8s.KubectlOptions) error {
	...
	_, err := retry.DoWithRetryE(t, "waiting for pods to be recreated with new timestamp",
		...
		func() (string, error) {
			...
			// This is flipped to false if any one of the pod is not yet restarted.
			podsAreReady := true
			...
			for _, pod := range pods {
				if !pod.CreationTimestamp.Time.After(initialTimestamp) {
					podsAreReady = false
					return "", fmt.Errorf("pod %s is still running", pod.Name)
				}
			}
			if podsAreReady {
				return "", nil
			}
			return "", fmt.Errorf("no pods have been recreated with a new timestamp yet")
		})
	return err
}
...
func (r *Region) ValidateMultiRegionSetup(t *testing.T) {
	...
	for _, cluster := range Clusters {
		...
		for _, clusterRegion := range expectedRegions {
			require.Contains(t, nodeStatus, clusterRegion)
		}
		require.NotContains(t, nodeStatus, "false")
	}
}
...
// single k3d cluster.
func CreateK3DCluster(t *testing.T, index int) error {
	cmd := shell.Command{
		Command: "make",
		Args: []string{
