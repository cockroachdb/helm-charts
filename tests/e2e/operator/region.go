...
func (r *Region) InstallCharts(t *testing.T, cluster string, index int) {
	...
	// when provider is k3d and custom labels are enabled, use localityMappings.
	if r.Provider == "k3d" && r.UseCustomNodeLabels {
		crdbOp = PatchHelmValues(map[string]string{
			"cockroachdb.crdbCluster.localityMappings[0].nodeLabel":                  "topology.kubernetes.io/dc",
			"cockroachdb.crdbCluster.localityMappings[0].localityLabel":              "region",
			"cockroachdb.crdbCluster.localityMappings[1].nodeLabel":                  "topology.kubernetes.io/rack",
			"cockroachdb.crdbCluster.localityMappings[1].localityLabel":              "zone",
			"cockroachdb.crdbCluster.topologySpreadConstraints[0].topologyKey":       "topology.kubernetes.io/rack",
			"cockroachdb.crdbCluster.topologySpreadConstraints[0].maxSkew":           "1",
			"cockroachdb.crdbCluster.topologySpreadConstraints[0].whenUnsatisfiable": "DoNotSchedule",
		})
	}
