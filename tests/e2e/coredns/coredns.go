...
func CoreDNSService(IpAddress *string, annotations map[string]string) *corev1.Service {
	...
	// For backward compatibility, set the LoadBalancerIP field only if not using annotation approach.
	// Modern Kubernetes and cloud providers prefer annotations over the deprecated LoadBalancerIP field.
	if IpAddress != nil && len(serviceAnnotations) == 0 {
...
func CoreDNSDeployment(replicas int32) *appsv1.Deployment {
	...
	memoryLimit := resource.MustParse("170Mi")
