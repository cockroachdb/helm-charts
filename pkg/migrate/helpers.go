...
func buildHelmValuesFromHelm(
	...
	// Add log.persistentVolume if a dedicated logsdir PVC was detected in the StatefulSet.
	if input.logsStorePVCSpec != nil {
		logPV := map[string]interface{}{
			"enabled": true,
			"path":    input.logsStorePVCSpec.MountPath,
		}
		if vct := input.logsStorePVCSpec.VolumeClaimTemplate; vct != nil {
			if storage := vct.Spec.Resources.Requests[corev1.ResourceStorage]; !storage.IsZero() {
				logPV["size"] = storage.String()
			}
			if vct.Spec.StorageClassName != nil && *vct.Spec.StorageClassName != "" {
				logPV["storageClassName"] = *vct.Spec.StorageClassName
			}
		}
		crdbCluster["log"] = map[string]interface{}{
			"persistentVolume": logPV,
		}
	}
...
func generateParsedMigrationInput(
	...
	// If present, populate logsStorePVCSpec so the operator values.yaml gets log.persistentVolume.* set
