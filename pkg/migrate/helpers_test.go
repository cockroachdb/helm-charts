...
func TestBuildHelmValuesFromHelm_WALFailover(t *testing.T) {
	...
	testCases := []struct {
		...
		},
		{
			...
			expectedPath:   "",
			expectedStatus: "", // No WAL failover spec should be created when disabled
		},
		{
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.walFailoverArg != "" && strings.Contains(tc.walFailoverArg, "path=") {
				...
				// When disabled, walFailoverSpec should be nil (not created)
...
func TestBuildWalFailoverSpec(t *testing.T) {
	...
	testCases := []struct {
		...
		},
		{
			...
			expectedStatus:    "", // No status expected when walFailoverSpec is nil
			expectedPath:      "",
			shouldHaveWalSpec: false, // No WAL failover spec should be created when disabled
		},
		{
					...
					Containers: []corev1.Container{
						{
							...
							Args: []string{
								"shell",
								"-ecx",
								"exec /cockroach/cockroach start --join=cockroachdb-0.cockroachdb.default.svc.cluster.local:26257 --advertise-host=$(hostname).cockroachdb.default.svc.cluster.local --certs-dir=/cockroach/cockroach-certs/ --http-port=8080 --port=26257",
							},
...
func TestBuildHelmValuesFromHelm_WithLogsPVC(t *testing.T) {
	...
	require.True(t, ok, "logsStore section should be present")

	assert.Equal(t, "/cockroach/logs", logsStore["mountPath"])
	vct, ok := logsStore["volumeClaimTemplate"].(*corev1.PersistentVolumeClaim)
	require.True(t, ok, "volumeClaimTemplate should be a *corev1.PersistentVolumeClaim")
	assert.Equal(t, "standard", *vct.Spec.StorageClassName)
	assert.Equal(t, resource.MustParse("50Gi"), vct.Spec.Resources.Requests[corev1.ResourceStorage])
}
...
func TestBuildNodeSpecFromHelm_WithWalFailover(t *testing.T) {
	...
	testCases := []struct {
		...
		},
		{
			...
			hasWalFailover:  false, // When disabled, no walFailoverSpec should be present
			walFailoverSpec: nil,   // No spec when disabled
		},
	}
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			assert.Equal(t, sts.Spec.Template.Labels, nodeSpec.PodTemplate.Metadata.Labels)
			assert.Equal(t, sts.Spec.Template.Annotations, nodeSpec.PodTemplate.Metadata.Annotations)
