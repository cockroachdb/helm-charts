...
> **Note on paths in this guide**: the `git checkout v*-preview+*` commands below
> land you on release commits that pre-date the folder rename (`cockroachdb-parent/`
> → `cockroachdb-operator/`). At those revisions the umbrella folder is called
> `cockroachdb-parent/`, so the subsequent `helm upgrade ./cockroachdb-parent/…`
> commands are correct for the checked-out working tree — do not rename them to
> `./cockroachdb-operator/…` when following this guide against a historical
> release.
...
git checkout v25.4.3-preview+1
...
git checkout v25.4.3-preview+2
...
git checkout v26.1.1-preview+2
